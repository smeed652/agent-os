...
class TDDValidator {
  ...
  async checkProjectStructure() {
    ...
    if (missingDirs.length > 0) {
      this.report.issues.push(`Missing required directories: ${missingDirs.join(', ')}`);
    }

    // Check for test configuration files
    const testConfigs = ['jest.config.js', 'jest.config.ts', 'cypress.config.js', 'cypress.config.ts'];
    const hasTestConfig = testConfigs.some(config => fs.existsSync(path.join(this.projectRoot, config)));
    
    if (!hasTestConfig) {
      this.report.issues.push('No test configuration found (Jest, Cypress, etc.)');
    }
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      const testScripts = packageJson.scripts ? Object.keys(packageJson.scripts).filter(key => key.includes('test')) : [];
    ...
    try {
      ...
      if (coverageMatch) {
        ...
        if (this.report.testCoverage >= 90) {
          ...
          this.report.recommendations.push(`Increase test coverage from ${this.report.testCoverage}% to at least 90%`);
        }
      }
    } catch (error) {
      this.report.issues.push('Could not run test coverage - check test configuration');
  ...
  async validateTestQuality() {
    ...
    if (qualityIssues > 0) {
      this.report.recommendations.push(`Fix ${qualityIssues} test quality issues`);
    }
  }
  ...
  async validateTDDWorkflow() {
    ...
    sourceFiles.forEach(sourceFile => {
      const correspondingTestFile = this.findCorrespondingTestFile(sourceFile, testFiles);
    ...
    if (tddCompliance < totalSourceFiles) {
      this.report.recommendations.push(`Improve TDD workflow - ${totalSourceFiles - tddCompliance} files need tests written first`);
    }
  }
  ...
  findSourceFiles() {
    ...
    searchDirs.forEach(dir => {
      if (fs.existsSync(path.join(this.projectRoot, dir))) {
        this.walkDir(path.join(this.projectRoot, dir), (filePath) => {
          if (filePath.match(/\.(js|ts|jsx|tsx)$/) && !filePath.includes('.test.') && !filePath.includes('.spec.')) {
  ...
  findCorrespondingTestFile(sourceFile, testFiles) {
    ...
    const sameDirTest = testFiles.find(testFile => 
      path.dirname(testFile) === sourceDir && 
    ...
    const testsDirTest = testFiles.find(testFile => 
      testFile.includes(sourceName) && 
  ...
  analyzeTestFile(content) {
    ...
    const poorTestNames = content.match(/it\s*\(\s*['"`](should work|test|works?|ok)['"`]/gi);
    if (poorTestNames) {
      issues.push(`Poor test names found: ${poorTestNames.length} tests have vague names`);
    }
    
    // Check for tests without assertions
    const testBlocks = content.match(/it\s*\([^)]+\)\s*\{[\s\S]*?\}/g);
    if (testBlocks) {
      const noAssertions = testBlocks.filter(test => {
        const testContent = test.toLowerCase();
        return !testContent.includes('expect') && !testContent.includes('assert') && !testContent.includes('should');
      });
      if (noAssertions.length > 0) {
        issues.push(`${noAssertions.length} tests appear to have no assertions`);
      }
    }
    
    // Check for test interdependence
    if (content.includes('beforeAll') || content.includes('afterAll')) {
      issues.push('Tests may have interdependence - review beforeAll/afterAll usage');
    }
  ...
  async generateReport() {
    ...
    if (this.report.overallStatus === 'TDD COMPLIANT') {
      console.log('🎉 Congratulations! Your project follows TDD practices correctly.');
    } else if (this.report.overallStatus === 'PARTIALLY COMPLIANT') {
      console.log('⚠️  Your project has some TDD compliance issues that should be addressed.');
    } else {
      console.log('🚨 Your project needs significant improvements to follow TDD practices.');
    }
    
    // Save report to file
    const reportPath = path.join(this.projectRoot, 'tdd-validation-report.json');
