...
class ValidatorTester {
  ...
  getTestScenarios() {
    ...
    return fs
      ...
      .filter((item) =>
        fs.statSync(path.join(this.testsDir, item)).isDirectory()
      )
  ...
  validateSpecDirectory(specDirName, specPath) {
    ...
    const requiredFilesPass =
      result.validations.required_files.status === "PASS";

    if (requiredFilesPass) {
      result.validations.content_structure =
        this.validateContentStructure(specPath);
      result.validations.content_quality =
        this.validateContentQuality(specPath);
    }
  ...
  validateDirectoryStructure(specDirName) {
    ...
    if (match) {
      ...
      checks.push({
        ...
        details: dateValid
          ? `Date '${datePart}' is valid ISO format`
  ...
  validateContentStructure(specPath) {
    ...
    const specLiteContent = this.readFileContent(
      path.join(specPath, "spec-lite.md")
    );
    const hasSpecLiteHeader = specLiteContent.includes("# Spec Summary (Lite)");
    const specLiteLength = specLiteContent
      .split("\n")
      .filter((line) => line.trim() && !line.startsWith("#")).length;
    const specLiteValid =
      hasSpecLiteHeader && specLiteLength >= 1 && specLiteLength <= 3;
    ...
    if (fs.existsSync(techSpecPath)) {
      ...
      const hasTechHeader = techSpecContent.includes(
        "# Technical Specification"
      );
      const hasTechRequirements = techSpecContent.includes(
      ...
      checks.push({
        ...
        details:
          hasTechHeader && hasTechRequirements
    ...
    const hasAllStatusFields = statusFields.every((field) =>
      statusContent.includes(field)
    );
  ...
  validateContentQuality(specPath) {
    ...
    if (userStoriesMatch) {
      ...
      const hasProperFormat =
        userStoriesText.includes("As a ") && userStoriesText.includes("I want");

      checks.push({
        check: "User stories format and count (1-3)",
        result:
          storyCount >= 1 && storyCount <= 3 && hasProperFormat
            ? "PASS"
            : "FAIL",
        details:
          storyCount >= 1 && storyCount <= 3 && hasProperFormat
    ...
    // Check spec scope
    const specScopeMatch = specContent.match(
      /## Spec Scope\s*\n(.*?)(?=\n## |\n\n|$)/s
    );
    if (specScopeMatch) {
    ...
    const deliverablesMatch = specContent.match(
      /## Expected Deliverable\s*\n(.*?)(?=\n## |\n\n|$)/s
    );
    if (deliverablesMatch) {
      const deliverablesText = deliverablesMatch[1];
      const deliverableCount = (deliverablesText.match(/^\d+\./gm) || [])
        .length;
    ...
    const hasVerifyTests =
      tasksContent.includes("Verify") && tasksContent.includes("tests pass");

    checks.push({
      check: "Tasks follow TDD approach",
      result: hasWriteTests && hasVerifyTests ? "PASS" : "FAIL",
      details:
        hasWriteTests && hasVerifyTests
          ...
          if (check.result === "FAIL") {
            if (check.check.includes("spec-lite.md exists")) {
              recommendations.push({
                ...
                action:
                  "Use @~/.agent-os/instructions/create-spec.md to generate proper spec-lite.md",
              });
              ...
              recommendations.push({
                ...
                action:
                  "Use @~/.agent-os/instructions/create-spec.md to generate proper tasks.md",
              });
              ...
              recommendations.push({
                ...
                action:
                  "Use @~/.agent-os/instructions/create-spec.md to generate technical specification",
              });
            }
          }
        });
  ...
  compareResults(expected, actual) {
    ...
    if (expected.validation_status !== actual.validation_status) {
      differences.push(
        `Status mismatch: expected ${expected.validation_status}, got ${actual.validation_status}`
      );
      matches = false;
    }
    ...
    if (expected.summary && actual.summary) {
      if (expected.summary.failed_checks !== actual.summary.failed_checks) {
        differences.push(
          `Failed checks mismatch: expected ${expected.summary.failed_checks}, got ${actual.summary.failed_checks}`
        );
        matches = false;
      }
      ...
      if (expected.summary.passed_checks !== actual.summary.passed_checks) {
        differences.push(
          `Passed checks mismatch: expected ${expected.summary.passed_checks}, got ${actual.summary.passed_checks}`
        );
        matches = false;
      }
    }
    ...
    if (expected.validations && actual.validations) {
      Object.keys(expected.validations).forEach((validationType) => {
        ...
        if (expectedValidation.status !== actualValidation.status) {
          differences.push(
            `${validationType} status mismatch: expected ${expectedValidation.status}, got ${actualValidation.status}`
          );
          matches = false;
        }
      });
