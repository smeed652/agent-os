...
class BranchStrategyValidatorTests extends ValidatorTestFramework {
  ...
  async testBranchNamingValidation() {
    ...
    await this.runValidatorTest(BranchStrategyValidator, 'Validator instantiation', async (validator) => {
      this.assert(validator instanceof BranchStrategyValidator, 'Should create validator instance');
      this.assert(typeof validator.validateProject === 'function', 'Should have validateProject method');
      this.assert(typeof validator.validateBranchNaming === 'function', 'Should have validateBranchNaming method');
      
      return { status: 'PASS' };
    });

    // Test isGitRepository method
    await this.runValidatorTest(BranchStrategyValidator, 'Git repository detection', async (validator) => {
      // Test with current directory (which is a git repo)
      const isGitRepo = validator.isGitRepository('.');
      this.assert(typeof isGitRepo === 'boolean', 'Should return boolean for git repository check');
      
      // Test with non-existent directory
      const isNonGitRepo = validator.isGitRepository('/non/existent/path');
      this.assert(isNonGitRepo === false, 'Should return false for non-existent path');
  ...
  async testErrorHandling() {
    ...
    await this.runValidatorTest(BranchStrategyValidator, 'Non-git repository', async (validator) => {
      ...
      try {
        ...
        this.assert(error.message.includes('Not a Git repository'), 'Should throw appropriate error');
        return { error: error.message };
      }
    });

    // Test helper method functionality
    await this.runValidatorTest(BranchStrategyValidator, 'Helper method availability', async (validator) => {
      // Test that key methods exist
      this.assert(typeof validator.isGitRepository === 'function', 'Should have isGitRepository method');
      this.assert(typeof validator.getAllBranches === 'function', 'Should have getAllBranches method');
  ...
  async testEdgeCases() {
    ...
    await this.runValidatorTest(BranchStrategyValidator, 'Validator configuration', async (validator) => {
    ...
    await this.runValidatorTest(BranchStrategyValidator, 'Validation aggregation', async (validator) => {
    ...
    await this.runValidatorTest(BranchStrategyValidator, 'Validation creation helper', async (validator) => {
