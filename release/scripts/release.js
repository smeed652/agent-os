...
const IS_DRY_RUN =
  CLI_ARGS.includes("--dry-run") || process.env.DRY_RUN === "1";
...
function deriveTagFromVersion(version) {
  ...
  // For frequent releases, use timestamp suffix to avoid conflicts
  const timestamp = new Date()
    .toISOString()
    .replace(/[-:TZ.]/g, "")
    .slice(0, 14);
  const buildTag = `${baseTag}+build.${timestamp}`;

  // Ensure the build tag doesn't exist either
  if (!tagExists(buildTag)) return buildTag;

  // Fallback: add random suffix for extremely frequent releases
  const randomSuffix = Math.random().toString(36).substring(2, 8);
  return `${baseTag}+build.${timestamp}.${randomSuffix}`;
}
...
async function release() {
  ...
  try {
    ...
    if (
      !runCommand(
        "npm ci",
        "Installing hello-world-app dependencies",
        HELLO_WORLD_DIR
      )
    ) {
      logError(
        "Hello World app dependency installation failed; aborting release"
      );
      process.exit(1);
    }
    if (NO_LINT) {
    ...
    if (NO_VALIDATORS) {
      ...
      if (
        !runCommand("npm run validate:all", "Running comprehensive validation")
      ) {
    ...
    logStep(
      "🏗️",
      USE_BUILT_APP
        ? "Using existing Hello World app build"
    ...
    if (!USE_BUILT_APP) {
      ...
      const buildEnv =
        `${NO_LINT ? "SKIP_LINT=1 " : ""}${NO_TESTS ? "SKIP_TESTS=1 " : ""}`.trim();
      const buildCmd =
        buildEnv.length > 0 ? `${buildEnv} npm run build` : "npm run build";
...
`;
    ...
    log(
      `The release package is available in the '${RELEASE_DIR}/' directory.`,
      "cyan"
    );
    log(
      'Users can install it with "npm install" in the release directory.',
      "cyan"
    );
  } catch (error) {
