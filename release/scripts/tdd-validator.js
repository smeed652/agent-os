...
class TDDValidator {
  ...
  analyzeTestFile(content) {
    ...
    const linesWithExpect = (contentWithoutComments.match(/expect\s*\(/g) || []).length;
    const linesWithAssert = (contentWithoutComments.match(/assert\s*\(/g) || []).length;
    ...
    if (testCount > 0 && totalAssertions < testCount) {
      ...
      issues.push(`${testsWithoutAssertions} tests appear to have no assertions`);
    }
